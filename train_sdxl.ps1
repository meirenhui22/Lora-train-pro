...
$pretrained_model = "E:\\lora-train\\models\sdxl\\基础模型XL _xl_1.0.safetensors" # base model path | 底模路径
...
# 新增：缓存和精度设置
$cache_latents = 1 # 1启用latents缓存，0禁用，对应参数--cache_latents
$mixed_precision = "bf16" # 混合精度训练设置，如"bf16"、"fp16"或"no"
$save_precision = "bf16" # 模型保存精度设置，如"bf16"、"fp16"或"float"
...
# Learning rate | 学习率
$lr = "8e-4" # learning rate | 学习率，在分别设置下方 U-Net 和 文本编码器 的学习率时，该参数失效
$unet_lr = "1e-5" # U-Net learning rate | U-Net 学习率
$text_encoder_lr = "1e-4" # Text Encoder learning rate | 文本编码器 学习率
...
$output_name = "sdxl-houtu-v1" # output model name | 模型保存名称
...
$sample_prompts = "./prompts/sd-prompts.txt" #prompt文件路径
