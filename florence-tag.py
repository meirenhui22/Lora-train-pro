...
if torch.cuda.is_available():
    ...
    print("未检测到可用GPU，原因可能：")
    print("1. 未安装带CUDA的PyTorch")
    print("2. CUDA驱动未安装或版本不匹配")
    print("3. 系统中无NVIDIA GPU")
    device = "cpu"
...
# 新增：记录并打印GPU使用状态
if torch.cuda.is_available():
    print(f"检测到可用GPU：{torch.cuda.get_device_name(0)}，将使用GPU进行处理")
    print(f"GPU内存：{torch.cuda.get_device_properties(0).total_memory / 1024**3:.2f} GB")  # 显示GPU内存
else:
    print("未检测到可用GPU，将使用CPU进行处理（处理速度可能较慢）")
